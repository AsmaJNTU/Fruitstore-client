...
import { addDryFruits, getDryFruits,deleteDryFruits,editDryFruits } from "../../api/Api";
import {toast} from 'sonner'
//import BasicFruits from "../BasicFruits";
const AdminDryFruits = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addDryFruits(dryfruits)
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editDryFruits(product, currentProduct._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteDryFruits(id);
      if (response.status === 200) {
  ...
  if (!dryfruits || dryfruits.length === 0) {
    return (
      <>
        <div className="w-screen h-[90vh] flex flex-col justify-center items-center">
