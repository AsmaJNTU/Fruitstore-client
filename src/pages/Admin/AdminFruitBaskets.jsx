...
import { addFruitBaskets, getFruitBaskets,deleteFruitBaskets,editFruitBaskets } from "../../api/Api";
import {toast} from 'sonner'
//import BasicFruits from "../BasicFruits";
const AdminFruitBaskets = () => {
  ...
  const handleAdd = async (e) => {
    ...
    console.log(fruitbaskets);
    try {
      const response = await addFruitBaskets(fruitbaskets)
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editFruitBaskets(product, currentProduct._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteFruitBaskets(id);
      if (response.status === 200) {
  ...
  if (!fruitbaskets || fruitbaskets.length === 0) {
    return (
      <>
        <div className="w-screen h-[90vh] flex flex-col justify-center items-center">
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start">
      <div className="w-full flex flex-row justify-between items-center my-4 shadow-md rounded-md p-1 border">
        <AdminPageHeader title="Fruit Baskets" />
        <button
      ...
      <table className="w-full h-full border-collapse border shadow-lg rounded-md">
        ...
        <tbody>
          {fruitbaskets.map((product, index) => (
            <tr key={index}>
...
export default AdminFruitBaskets;