...
import { addExoticFruits, getExoticFruits,deleteExoticFruits,editExoticFruits } from "../../api/Api";
import {toast} from 'sonner'
//import BasicFruits from "../BasicFruits";
const AdminExoticFruits = () => {
  //null -> products[] | Store the data
  const [exoticfruits, setExoticFruits] = useState(null);
  ...
  const fetchData = async () => {
    try {
      const res = await getExoticFruits();
      if (res.status === 200) {
        console.log(res.data);
        setExoticFruits(res.data);
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editExoticFruits(product, currentProduct._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteExoticFruits(id);
      if (response.status === 200) {
  ...
  if (!exoticfruits || exoticfruits.length === 0) {
    return (
      <>
        <div className="w-screen h-[90vh] flex flex-col justify-center items-center">
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start">
      ...
      <table className="w-full h-full border-collapse border shadow-lg rounded-md">
        ...
        <tbody>
          {exoticfruits.map((product, index) => (
            <tr key={index}>
